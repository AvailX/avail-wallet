...
export async function createWalletConnectDialog(
  ...
  return new Promise((resolve, reject) => {
    ...
    // dialogConfig.approveEventString
...
export class WalletConnectManager {
  ...
  async setup() {
    ...
    this.theWallet = await Web3Wallet.init({
      ...
      metadata: {
        name: "Avail",
        description:
          "Frictionless control of your money and data privately on Aleo.",
        url: "avail.global",
        icons: [],
      },
    });
    ...
    try {
      this.clientId =
        await this.theWallet.engine.signClient.core.crypto.getClientId();
    ...
    this.theWallet.engine.signClient.events.on(
      "session_ping",
      this.onSignClientPing
    );
  ...
  async close() {
    ...
    this.theWallet?.engine.signClient.events.off(
      "session_ping",
      this.onSignClientPing
    );
  ...
  private async onSessionProposal(proposal: Web3WalletTypes.SessionProposal) {
    ...
    try {
      ...
      //   wcRequest
      // );
      const response = await createWalletConnectDialog(
        {
          onApprove: async () => {
            console.log("Wallet connect approved");
            const supportedNamespaces = {
              aleo: {
                chains: [this.aleoWallet.chainName()],
                methods: this.aleoWallet.chainMethods(),
                events: this.aleoWallet.chainEvents(),
                accounts: [
                  `${this.aleoWallet.chainName()}:${this.aleoWallet.getAddress()}`,
                ],
              },
            };
            const session = await this.theWallet.approveSession({
              id: proposal.id,
            ...
            sessionStorage.setItem(session.topic, JSON.stringify({
              name: metadata.name,
              description: metadata.description,
              url: metadata.url,
          ...
          approveEventString: "connect-approved",
          rejectEventString: "connect-rejected",
          requestIdentifier: `sign-${metadata.name}`,
        },
        wcRequest
      );

      console.log("Wallet connect dialog result:", response);
      ...
      console.log("Session proposal processing completed.");
    }
  }
  ...
  private async onSessionRequest(requestEvent: Web3WalletTypes.SessionRequest) {
    try {
      ...
      const requestSession = this.theWallet.engine.signClient.session.get(
        requestEvent.topic
      );
      ...
      let response: JsonRpcResult | JsonRpcError = formatJsonRpcError(
        requestEvent.id,
      ...
      await this.theWallet?.respondSessionRequest({
        topic,
        response: formatJsonRpcError(requestEvent.id, (error as Error).message),
      });
    } finally {
      console.log();
