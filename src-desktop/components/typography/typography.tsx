...
const ResponsiveTypography = (variant: string) => styled(Typography)(({ theme }) => {
	...
	return {
		// Title
		fontSize: variant === 'largetitle' && xl ? '8rem'
			: variant === 'largetitle' && lg ? '7rem'
				: variant === 'largetitle' && md ? '6rem'
					: variant === 'largetitle' ? '5rem' // Default for 'largetitle'
						: variant === 'title' && xl ? '4rem'
...
export const LargeTitleText = styled(ResponsiveTypography('largetitle'))(({ theme }) => ({
	fontWeight: 500,
	lineHeight: 0.95,
}));
