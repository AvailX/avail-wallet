...
import { type AvailError } from "../../../src-desktop/types/errors";
import {
  aleoChain,
  AleoEvents,
  AleoMethod,
...
export class AleoWallet {
  ...
  private async handleGetEvent(
    ...
    const metadata = getDappMetadata(requestEvent.topic);
    const request = requestEvent.params.request.params as GetEventRequest;
    ...
    if (checkWindow("wallet-connect")) {
      ...
      const unlistenCreated = await webview.once(
        "tauri://created",
        async () => {
    ...
    return new Promise(async (resolve, reject) => {
      const unlistenApproved = once("get-event-approved", async (response) => {
      ...
      const unlistenRejected = once("get-event-rejected", async (response) => {
  ...
  private async handleGetEvents(
    ...
    const metadata = getDappMetadata(requestEvent.topic);
    const request = requestEvent.params.request.params as GetEventsRequest;
    ...
    return new Promise(async (resolve, reject) => {
      const unlistenApproved = once("get-events-approved", async (response) => {
        const unlisten = await unlistenApproved;
        unlisten();
      ...
      const unlistenRejected = once("get-events-rejected", async (response) => {
        const unlisten = await unlistenRejected;
        unlisten();
        await webview.destroy();
        reject(formatJsonRpcResult(requestEvent.id, response));
  ...
  private async handleGetRecords(
    ...
    const metadata = getDappMetadata(requestEvent.topic);
    const request = requestEvent.params.request.params as GetRecordsRequest;
