  ...
  invoke<string>(get_os_function_name(function_to_call), {
    ...
    .then((response) => {
      if (response === 'Key stored') {
        ...
        setMessage(
          'Problem when creating account, apologies. Please try again.'
        );
        setErrorAlert(true);
      }
    })
...
export async function register_seed_phrase(
  setErrorAlert: React.Dispatch<React.SetStateAction<boolean>>,
  setMessage: React.Dispatch<React.SetStateAction<string>>,
  username: string | undefined,
  password: string,
...
export async function import_wallet(
  username: string | undefined,
  password: string,
  authType: boolean,
  private_key: string,
