...
const Header = () => {
	...
	return (
		<mui.Box sx={{
			display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignSelf: 'center', alignItems: 'center', width: '90%',
		}}>
			<mui.Box className={path == '/contracts' ? 'hex2' : 'hex1'} onClick={() => {
				navigate('/account');
			}}>
			...
			<mui.Box sx={{display: 'flex', flexDirection: 'row', gap: '17%'}}>
				<NotificationsIcon sx={{width: '30px', height: '30px', color: path == '/contracts' ? '#fff' : '#081424'}} onClick={() => {
