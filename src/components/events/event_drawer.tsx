...
// Services
import { getAvailEvent } from "../../../src-desktop/services/events/get_events";
...
// Util
import { type SuccinctAvailEvent } from "../../../src-desktop/types/avail-events/event";
...
const EventDrawer: React.FC<EventDrawerProps> = ({ open, onClose, event }) => {
  const [fullEvent, setFullEvent] = React.useState<AvailEvent | undefined>(
    undefined
  );
        ...
        >
          ...
          >
            ...
            >
              <SubMainTitleText sx={{ color: "#fff" }}>
                {type == EventType.Send && to
                  ...
                  : type == EventType.Send && !to
                    ? "Sent"
                    : type == EventType.Receive && from
                      ? `Received from @${formatLongString(from)}`
                      : type == EventType.Receive && !from
                        ? "Received"
                        : type == EventType.Execute && programId && functionId
                          ? `Executed ${programId}/${functionId}`
                          : type == EventType.Execute && functionId
                            ? `Executed ${functionId}`
            ...
            {amount && programId && (
              ...
              >
                <BodyText500 sx={{ color: "#a3a3a3" }}>Amount:</BodyText500>
            ...
            {fee && (
              ...
              >
                <BodyText500 sx={{ color: "#a3a3a3" }}>Fee:</BodyText500>
                <BodyText sx={{ color: "#fff", ml: "14%" }}>{fee}</BodyText>
              </Box>
            )}
            {message && (
              <Box
              ...
              >
                <BodyText500 sx={{ color: "#a3a3a3" }}>Message:</BodyText500>
            ...
            >
              <BodyText500 sx={{ color: "#a3a3a3" }}>Created:</BodyText500>
            ...
            {/* Fee Transition */}
            {fee_transition && <Transition event_transition={fee_transition} />}
