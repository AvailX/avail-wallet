...
import { Box, Chip, keyframes, LinearProgress, styled } from '@mui/material';
...
function Home() {
  ...
  const [localScan, setLocalScan] = useState<boolean>(false);
  ...
  const RotatingSyncIcon = styled(SyncIcon)(({ theme }) => ({
    color: '#00FFAA',
    width: '40px',
    height: '30px',
  ...
  return (
    <Layout>
      ...
      {!loading && (
        ...
        >
          {scanInProgress && (
            <Box sx={{ width: '100%', bgcolor: '#00FFAA', height: '30px' }}>
              <SmallText400 sx={{ color: '#111111' }}>
                {' '}
                {t('home.scan.progress')} {scanProgressPercent?.toString()}%
                {t('home.scan.complete')}
              </SmallText400>
            </Box>
          )}
          {scanInProgress && (
            <LinearProgress variant='determinate' value={scanProgressPercent} />
          )}
          ...
          >
            ...
            {networkStatus !== NetworkStatus.Up && (
              <WarningIcon
                sx={{
                  color:
                    networkStatus === NetworkStatus.Warning
                      ? '#FFA500'
                      : '#FF0000',
                  ml: '1%',
                  cursor: 'pointer',
          ...
          >
            ...
            <Box
              sx={{
                background: 'linear-gradient(90deg, #1E1D1D 0%, #111111 100%)',
                display: 'flex',
                flexDirection: 'column',
                p: 2,
                borderRadius: '30px',
          ...
          <AssetDrawer
            ...
            onClose={handleAssetDrawerClose}
            asset={asset}
            address={address}
            username={username}
          />
