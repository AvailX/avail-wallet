...
function Nfts() {
  ...
  const [airdropNfts, setAirdropNfts] = useState<Collection[]>([]);
  ...
  const [selectedAirdropNft, setSelectedAirdropNft] =
    useState<Collection>(testCollection);
    ...
    <Layout>
      ...
      >
        ...
        >
          ...
          <Title2Text sx={{ color: '#828282' }}>
            Own NFTs in private for the first time.
          </Title2Text>
        </Box>
        <Box
          sx={{
        ...
        >
          ...
          {airdropNfts.length > 0 && (
            <Grid
              container
              spacing={2}
              sx={{ width: '90%', alignSelf: 'center', mt: '5%' }}
            >
              {airdropNfts.map((collection, index) => (
                <Grid item key={index}>
                  <AirdropNft
                    collection={collection}
                    setCollection={setSelectedAirdropNft}
                  />
                </Grid>
              ))}
            </Grid>
          ...
          {nfts.length === 0 && airdropNfts.length === 0 && !loading && (
            ...
            >
              ...
              >
                ...
                >
                  <ErrorOutlineIcon
                    sx={{ width: '150px', height: '150px', color: '#2A2A2A' }}
                  />
                  <LargeTitleText sx={{ color: '#2A2A2A', ml: '5%' }}>
                    No NFTs Yet
                  </LargeTitleText>
                </Box>
                <Box
                ...
                >
                  ...
                  >
                    Go to the In-App quests to earn NFTs
                  </SubtitleText>
                  <SubtitleText
                    sx={{ color: '#7b7b7b', cursor: 'pointer' }}
                    onClick={() => {
                      handleXlink('https://x.com/AvailWallet');
                    }}
                  >
                    Or go to @AvailWallet on X for more NFTs
                  </SubtitleText>
                </Box>
              </Box>
            </Box>
