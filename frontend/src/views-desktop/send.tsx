...
//types
import { TransferRequest, TransferType } from "../types/transfer_props/tokens";
...
function Send() {
    ...
    const handleTransfer = async () => {
      ...
      if (isPrivateTransferFrom && isPrivateTransferTo ){
       ...
       if (amount === undefined || recipient === "" || token === "" ){
        ...
        setErrorAlert(true);
        return;
       }else if (amount > privateBalance && isPrivateTransferFrom){
        setMessage(t("send.messages.error.insufficient-private-amount"));
        setErrorAlert(true);
        return;
       }else if (amount > publicBalance && !isPrivateTransferFrom){
    ...
    return (
        <Layout>
            ...
            {/* Transfer Dialog */}
            <TransferDialog isOpen={TransferDialogOpen} onRequestClose={()=>setTransferDialogOpen(false)} request={request}/>
            <MiniDrawer/>
            <mui.Box sx={{width:'100%',height:'100%',display:'flex',justifyContent:'center',alignContent:'center'}}>
          <mui.Box sx={{display:'flex',width:'45%',bgcolor:'#00A07D',borderRadius:9,mt:'12%'}}>
           <mui.Box sx={{display:'flex',flexDirection:"column",alignSelf:'center',width:'100%',borderRadius:9, backdropFilter: 'blur(10px)',  
          ...
          p:3,
            ...
            <mui.Box sx={{display:'flex',flexDirection:'column',mt:'2%'}}>
              <mui.Box sx={{display:'flex',flexDirection:'row',justifyContent:'space-between',alignSelf:'center',width:'80%'}}>
                ...
                <SmallText400 sx={{color:'#fff'}}>{t("send.fee")}: 0.29</SmallText400>
                 ...
                 {/* public balance*/}
                 <mui.Box sx={{display:'flex',flexDirection:'row',alignSelf:'center',width:'80%'}}>
            ...
            <mui.TextField
                ...
                sx={{
                  ...
                  InputLabelProps={{ style: { color: "#fff" } }}
            />
            <SettingsComponent onTransferFromToggle={(val)=>setIsPrivateTransferFrom(val)} onTransferToToggle={(val)=>setIsPrivateTransferTo(val)} onFeeToggle={(val)=>setIsPrivateFee(val)}/>
           <mui.Button
