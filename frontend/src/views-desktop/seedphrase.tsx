...
const SeedPhrase = () => {
	const [revealedWord, setRevealedWord] = React.useState<string | undefined>(undefined);
	...
	return (
		<Layout>
			...
			<img src={a_logo} alt='aleo logo' style={{
				width: '60px', height: '60px', marginTop: '20px', marginLeft: '20px',
			}} />
			...
			<mui.Grid container spacing={1} sx={{
				marginTop: '25px', alignSelf: 'center', bgcolor: '#1E1D1D', borderRadius: '10px', width: '85%', padding: '5%', justifyContent: 'center', mb: '1%', alignItems: 'center', position: 'relative',
			}}>
				...
				{secretWords.map((word, index) => (
					<mui.Grid
						key={index}
						sx={{
							color: '#fff', m: '2%', bgcolor: '#3E3E3E', borderRadius: '10px', padding: '1%', textAlign: 'center',
						}}
						item
			...
			<mui.Box sx={{
				display: 'flex', justifyContent: 'flex-end', alignSelf: 'center', right: 0, width: '85%',
			}}>
			...
			<CTAButton text={t('seedphrase.continue')} onClick={() => {
