...
// Components
import {
	ExpandLess, ExpandMore, Settings as SettingIcon, VpnKey, Lock, Info, Security, Polyline, RemoveCircleOutline,
...
// Typography
import {
	Title2Text, SubMainTitleText, SubtitleText, BodyText, BodyText500, SmallText400,
...
// Services
import {
	getUsername, updateUsername, get_address, getLastSync, getLanguage, getNetwork, updateBackupFlag, getBackupFlag,
...
function Settings() {
	...
	const HiddenItem: React.FC<{param: string; label: string}> = ({param, label}) => (
		<mui.Box>
			<mui.Box sx={{display: 'flex', flexDirection: 'row'}}>
				<STButton text={param == '' ? 'Get and Decrypt' : (revealAll ? 'Lock' : 'Unlock')} onClick={param == '' ? (() => {
					label === 'Private Key' ? setPkOpen(true) : (label === 'Viewing Key' ? setVkOpen(true) : setSpOpen(true));
				}) : (() => {
				...
				{param !== '' && (
					...
					}} size='large' sx={{color: '#00FFAA', '&:hover': {bgcolor: mui.alpha('#3a3a3a', 0.8)}}}>
						<ContentCopyIcon fontSize='inherit' />
					</mui.IconButton>
				)}
			</mui.Box>
			<mui.Box sx={{
				mt: '2%', borderRadius: '10px', bgcolor: '#1E1D1D', justifyContent: 'space-between', mb: '1%', alignItems: 'center', position: 'relative', padding: 2, display: 'flex', flexDirection: 'row',
			}}>
	...
	return (
		<Layout>
			...
			<DeleteDialog isOpen={deleteOpen} onRequestClose={() => {
				setDeleteOpen(false);
			}} />
			<ViewKeyDialog isOpen={vkOpen} onRequestClose={() => {
				setVkOpen(false);
			}} setViewKey={setVk} />
			<PrivateKeyDialog isOpen={pkOpen} onRequestClose={() => {
				setPkOpen(false);
			}} setPrivateKey={setPk} />
			<SeedPhraseDialog isOpen={spOpen} onRequestClose={() => {
				setSpOpen(false);
			}} setSeedPhrase={setSeedPhrase} />
			<ReAuthDialog isOpen={reAuthDialog} onRequestClose={() => {
				setReAuthDialog(false);
			}} />
			<MiniDrawer />
			<mui.Box sx={{
				display: 'flex', flexDirection: 'column', ml: '9%', mt: '2%',
			}}>
				<Title2Text sx={{color: '#FFF'}}>{t('settings.title')}</Title2Text>
				<SubtitleText sx={{color: '#a3a3a3'}}>{t('settings.subtitle')}</SubtitleText>
				<mui.Box sx={{
					display: 'flex', flexDirection: 'column', mt: '7%', width: '90%',
				}}>
					...
					<MenuSection title={t('settings.security.title')} IconComponent={<Security sx={{color: '#00FFAA'}} />}>
						...
						<BodyText sx={{color: '#fff', mt: '5%', '&:hover': {color: '#00FFAA', cursor: 'pointer'}}} onClick={() => {
							navigate('/privacy-policy');
						}}>Privacy Policy</BodyText>
						<BodyText sx={{
							color: '#fff', mt: '5%', mb: '3%', '&:hover': {color: '#00FFAA', cursor: 'pointer'},
						}} onClick={() => {
							navigate('/terms-of-service');
						}}>Terms of Service</BodyText>
					...
					<MenuSection title={t('settings.advanced.title')} IconComponent={<Polyline sx={{color: '#00FFAA'}} />} >
						...
						<mui.Box sx={{
							display: 'flex', flexDirection: 'row', mt: '2%', mb: '2%',
						}}>
							<STButton text={t('settings.advanced.full-resync')} onClick={() => {
								handleFullResync();
							}} />
							{scanInProgress ? (
								<mui.Box sx={{
									color: '#00FFAA', ml: '3%', display: 'flex', flexDirection: 'column',
								}}>
						...
						<mui.Box sx={{mt: '3%'}}>
							<STButton text={backup ? t('settings.advanced.disable-backup') : t('settings.advanced.enable-backup')} onClick={() => {
						...
						<mui.Box sx={{display: 'flex', flexDirection: 'column', mt: '1%'}}>
							<WhiteHueTextField sx={{width: '40%'}} inputProps={{style: {color: '#fff'}}} InputLabelProps={{style: {color: '#fff'}}} value={signMessage} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
								setSignMessage(event.target.value);
							}} label='message' />
						</mui.Box>
						<STButton text='Sign' onClick={() => {
							handleSign();
						}} />
						<SmallText400 sx={{
							color: '#fff', mt: '2%', mb: '2%', wordWrap: 'break-word',
						}}>{signature}</SmallText400>
						{/*
            ...
            <SmallText400 sx={{ color: '#fff', mt: '2%', mb: '2%', wordWrap: "break-word" }}>{verifyResult === undefined ? '' : verifyResult ? 'Signature verified successfully.' : 'Signature verification failed.'}</SmallText400>
                */}
					...
					<MenuSection title={t('settings.delete.title')} IconComponent={<RemoveCircleOutline sx={{color: '#D21C1C'}} />}>
						<STButton text={t('settings.delete.STButton')} onClick={() => {
							setDeleteOpen(true);
						}} />
					</MenuSection>
				</mui.Box>
