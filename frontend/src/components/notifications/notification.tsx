...
const Notification: React.FC<NotificationProps> = ({notif_type, nmessage, sub_message, link}) => {
	...
	return (
		<mui.Box sx={{
			display: 'flex', flexDirection: 'row', alignItems: 'center', bgcolor: mui.alpha('#3A3A3A', 0.4), borderRadius: '10px', p: '5px',
		}}>
			...
			<mui.Box sx={{display: 'flex', flexDirection: 'column'}}>
				...
				<mui.Box sx={{display: 'flex', flexDirection: 'row', gap: '1.5%'}} onClick={() => {
					if (!sub_message && link) {
						navigate(link);
					}
				}}>
