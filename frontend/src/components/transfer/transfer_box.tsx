...
const TransferBox: React.FC<TokenProps> = ({tokens, token, amount, setToken, setAmount}) => (
	<mui.Box sx={{
		display: 'flex', flexDirection: 'row', p: '2px', borderRadius: '10px', border: '1px solid #FFF', bgcolor: '#3E3E3E', alignItems: 'center', justifyContent: 'space-between', height: '55px',
	}}>
		<mui.Box sx={{display: 'flex', width: '100%', alignItems: 'center'}}>
			...
			<mui.Box sx={{
				...
				<mui.Divider orientation={'vertical'} sx={{
					bgcolor: '#A3A3A3', marginTop: '1%', height: 50, width: '1px', alignSelf: 'center', marginLeft: '10px',
				}}/>
			</mui.Box>
		</mui.Box>
		{/* --Amount Input-- */}
		<mui.Box sx={{
			display: 'flex', width: '60%', alignItems: 'center', justifyContent: 'flex-end', pr: '8%',
		}}>
			<input
				type='number'
				value={amount || ''}
