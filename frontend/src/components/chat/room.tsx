...
import { LastMessage } from "../../types/chat/messages";
...
const RoomItem: React.FC<RoomProps> = ({ room, username, address }) => {
    ...
    return (
        <mui.Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%', mt: '4%' }}>
            <mui.Box sx={{ display: 'flex', flexDirection: 'row', gap: '20px', alignItems: 'center' }}>
                { /* if last_message unseen then #00FFAA if seen then #273344  */}
                <mui.Avatar sx={{ bgcolor: seen ? "#00ffaa" : "#273344", width: '45px', height: '45px', color: seen ? "#273344" : '#00ffaa' }}>{initial}</mui.Avatar>
